...
export const dbOperations = {
  ...
  getContacts: async () => {
    if (isFirebaseConnected && db) {
      try {
        ...
        console.error("Firestore getContacts error:", err);
      }
    }
  ...
  deleteContact: async (id) => {
    if (isFirebaseConnected && db) {
      try {
        ...
        console.error("Firestore deleteContact error:", err);
      }
    }
  ...
  subscribeContacts: (onUpdate) => {
    ...
    // Local subscription
    localListeners.contacts.push(onUpdate);
    dbOperations.getContacts().then(onUpdate);
    return () => {
      localListeners.contacts = localListeners.contacts.filter(cb => cb !== onUpdate);
    };
  },
